...
if (!DB.settings.notifications) {
  ...
  navigator.serviceWorker.controller.postMessage({
    ...
    enabled:DB.settings.notifications.enabled,

    time:DB.settings.notifications.time,

    lastNotify:DB.settings.notifications.lastNotify || 0
  ...
  function loadSettingsUI() {
    ...
    if (!notifyToggle || !notifyTime) return;

    notifyToggle.checked = DB.settings.notifications.enabled;
    notifyTime.value = DB.settings.notifications.time;
  }
...
DB.settings.notifications.enabled = this.checked;
saveDB();
saveNotifySchedule();
  ...
  if (notifyTime) {
    ...
    notifyTime.addEventListener("change", function () {

DB.settings.notifications.time = this.value;
saveDB();
saveNotifySchedule();
  ...
  function checkNotification(){
    ...
    if(!DB.settings.notifications.enabled) return;
    ...
    const notifyTime = DB.settings.notifications.time;


    // ถึงเวลาที่ตั้งไว้
    if(currentTime === notifyTime){

      const cooldown = 90 * 60 * 1000; // 1 ชั่วโมง 30 นาที
      const current = Date.now();


      // กันยิงซ้ำ
      if(current - lastNotifyTime < cooldown){
        return;
      }
